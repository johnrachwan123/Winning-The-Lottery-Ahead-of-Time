...
class ResNext_8(Pruneable):

    def __init__(self, device="cuda", output_dim=2, input_dim=(1, 1, 1), **kwargs):
        super(ResNext_8, self).__init__(device=device, output_dim=output_dim, input_dim=input_dim, **kwargs)

        self.m = torch.hub.load('pytorch/vision:v0.5.0', 'resnext101_32x8d', pretrained=True)
...
if __name__ == '__main__':
    ...
    for test_batch in [mnist, cifar, imagenet]:
        conv = ResNext_8(output_dim=10, input_dim=test_batch.shape[1:], device=device)
