...
from ContainerLayers import ContainerLinear, ContainerConv2d
from L0_Layers import L0Linear, L0Conv2d
from constants import ZERO_SIGMA
    ...
    def get_num_nodes(self, init=False):
        ...
        for i, (name, module) in enumerate(self.named_modules()):
            if (hasattr(module, "weight") or hasattr(module, "weights")) and not ("Norm" in str(module.__class__)):
                ...
                counter += addition
            elif (hasattr(module,"weight_ih")) and not ("Norm" in str(module.__class__)):
                counter += module.weight_ih.shape[0]
                counter += module.weight_hh.shape[0]
        ...
        with torch.no_grad():
            ...
            if self.is_maskable:
                self.mask = {name + ".weight": torch.ones_like(module.weight.data).to(self.device) for name, module in
                             self.named_modules() if isinstance(module, (nn.Linear, nn.Conv2d, nn.Embedding))
                             }
                for name, module in self.named_modules():
                    if isinstance(module, nn.LSTMCell):
                        self.mask[name+".weight_ih"] = torch.ones_like(module.weight_ih).to(self.device)
                        self.mask[name+".weight_hh"] = torch.ones_like(module.weight_hh).to(self.device)
