...
from constants import RESULTS_DIR, OUTPUT_DIR, SNIP_BATCH_ITERATIONS
...
class SNIP(General):
    ...
    def get_weight_saliencies(self, train_loader):
        ...
        for idx, (data, label) in enumerate(train_loader, 1):
            ...
            inputs = data
            targets = Variable(label.view(-1)).cuda()
            outputs = net(inputs)
        ...
        for name, layer in net.named_modules():
            ...
            if name + ".weight" in self.model.mask:
                ...
                grads_abs[name + ".weight_ih"] = torch.abs(
                    layer.weight_ih.grad * (layer.weight_ih.data / (1e-8 + loss_sum.item())))
                grads_abs[name + ".weight_hh"] = torch.abs(
                    layer.weight_hh.grad * (layer.weight_hh.data / (1e-8 + loss_sum.item())))
